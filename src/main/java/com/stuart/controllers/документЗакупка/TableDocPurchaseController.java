...
import com.stuart.interfaces.impls.документы.HibernateDocPurchase;
...
public class TableDocPurchaseController {
    ...
    @FXML
    private void actionButtonPressed(ActionEvent actionEvent) throws SQLException {
    ...
    private void updateCountLabel() {
        labelCount.setText("Количество записей: "+listDocPurchaseImpl.getDocPurchaseList().size());
    }
    ...
    void actionSearch(ActionEvent event) throws SQLException {
        if ( txtSearch.getText().trim().length() == 0) {
            ...
            listDocPurchaseImpl.findText(txtSearch.getText());
//
